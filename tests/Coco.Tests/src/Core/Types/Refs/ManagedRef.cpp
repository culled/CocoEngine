...
namespace Coco::Core::Types::Ref
{
	TEST_CLASS(ManagedRefTests)
	{
		...
		TEST_METHOD(CreateAndDestroy)
		{
			...
			Assert::AreEqual(t.GetUseCount(), static_cast<uint64_t>(1));
		...
		TEST_METHOD(GetRef)
		{
...

			{
				...
				Assert::AreEqual(t.GetUseCount(), static_cast<uint64_t>(2));

				{
					...
					Assert::AreEqual(t.GetUseCount(), static_cast<uint64_t>(3));
				}
